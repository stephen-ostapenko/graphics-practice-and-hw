...
const char shrect_vertex_source[] = R"(
#version 330 core
...
void main() {
    ...
    pos = (VERTICES[gl_VertexID] + vec2(1.0, 1.0)) / 2;
}
)";

const char shrect_fragment_source[] = R"(
#version 330 core
...
void main() {
    out_color = vec4(texture(sh_map, pos).r, 0.0, 0.0, 1.0);
}
)";

const char shmap_vertex_source[] = R"(
#version 330 core
...
void main() {
    gl_Position = shadow_projection * view * model * vec4(in_position, 1.0);
}
)";

const char shmap_fragment_source[] = R"(
#version 330 core
...
try
{
    ...
    auto shrect_vertex_shader = create_shader(GL_VERTEX_SHADER, shrect_vertex_source);
    auto shrect_fragment_shader = create_shader(GL_FRAGMENT_SHADER, shrect_fragment_source);
    auto shrect_program = create_program(shrect_vertex_shader, shrect_fragment_shader);

    auto shmap_vertex_shader = create_shader(GL_VERTEX_SHADER, shmap_vertex_source);
    auto shmap_fragment_shader = create_shader(GL_FRAGMENT_SHADER, shmap_fragment_source);
    auto shmap_program = create_program(shmap_vertex_shader, shmap_fragment_shader);
    ...
    GLuint shrect_vao;
    glGenVertexArrays(1, &shrect_vao);
    glBindVertexArray(shrect_vao);
