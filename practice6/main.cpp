...
void main() {
    vec4 color = vec4(texture(render_result, vec2(texcoord.x, texcoord.y)).xyz, 1.0);
    if (mode == 1) {
    ...
    if (mode == 3) {
        color = vec4(texture(render_result, vec2(texcoord.x + sin(texcoord.y * 50.0 + time) * 0.01, texcoord.y)).xyz, 1.0);
    }
    out_color = color;
}
)";
...
int main() try
{
    ...
    while (running)
    {
        ...
        for (auto [f1, f2, mode] : {
            ...
            glClearColor((float)(mode == 1), (float)(mode == 2), (float)(mode == 3), 1.f);
            ...
            if (mode > 1) {
                view = glm::rotate(view, glm::pi<float>() / 2, {(float)(mode == 2), (float)(mode == 3), 0.f});
            }

            glm::mat4 projection = glm::perspective(glm::pi<float>() / 2.f, (1.f * width) / height, near, far);
            if (mode != 0) {
                double aspect_ratio = width / (float)height;
                projection = glm::ortho(-aspect_ratio, aspect_ratio, -1.0, 1.0, -5.0, 5.0);
            }
            ...
            glUniform2f(center_location, (float)f1 / 2, (float)f2 / 2);
