...
void main()
{
    ...
    vec3 another_light = (diffuse(direct_vec) + specular(direct_vec)) * point_light_color / (point_light_attenuation[0] + point_light_attenuation[1] * r + point_light_attenuation[2] * r * r);

    vec3 kek_sun_direction = normalize(sun_direction);
    vec3 color = ambient + (diffuse(kek_sun_direction) + specular(kek_sun_direction)) * sun_color + another_light;
...
int main() try {
    ...
    while (running) {
        ...
        glUniform3f(ambient_light_location, 0.3f, 0.3f, 0.3f);

        glUniform3f(sun_color_location, 1.f, 1.f, 1.f);
        float kek_time = 10 * (time - floor(time));
        if (floor(kek_time) == 0.f || floor(kek_time) == 2.f || floor(kek_time) == 4.f || floor(kek_time) >= 6.f) {
        //    glUniform3f(sun_color_location, 1.f, 0.f, 1.f);
        }
        glUniform3f(sun_direction_location, 0.0f, 3.0f, 3.0f);

        glUniform3f(point_light_position_location, 5 * sin(time), 3 * sin(10 * time), 5 * cos(time));
        ...
        float pos_x[3] = { -3.f, 0.f, 3.f };
        float pos_y[3] = { -3.f, 0.f, 3.f };
        float roughness[3][3] = {
            { 0.1f, 0.15f, 0.2f },
            { 0.25f, 0.3f, 0.35f },
            { 0.4f, 0.45f, 0.5f }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                // model = glm::translate(glm::mat4(1.f), {pos_x[j], pos_y[i], 0.f});
                model = glm::translate(glm::mat4(1.f), {pos_x[j] + cos(time), pos_y[i] + sin(time), 0.f});
                glUniformMatrix4fv(model_location, 1, GL_FALSE, reinterpret_cast<float *>(&model));

                glUniform1f(glossiness_location, 1 / roughness[i][j]);
                glUniform1f(roughness_location, roughness[i][j]);
