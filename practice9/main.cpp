...
int main() try
{
    ...
    auto cmp = [](int coord) {
        return [=](obj_data::vertex &a, obj_data::vertex &b) {
            return a.position[coord] < b.position[coord];
        };
    };

    float min_x = std::min_element(scene.vertices.begin(), scene.vertices.end(), cmp(0))->position[0];
    float max_x = std::max_element(scene.vertices.begin(), scene.vertices.end(), cmp(0))->position[0];
    float min_y = std::min_element(scene.vertices.begin(), scene.vertices.end(), cmp(1))->position[1];
    float max_y = std::max_element(scene.vertices.begin(), scene.vertices.end(), cmp(1))->position[1];
    float min_z = std::min_element(scene.vertices.begin(), scene.vertices.end(), cmp(2))->position[2];
    float max_z = std::max_element(scene.vertices.begin(), scene.vertices.end(), cmp(2))->position[2];
    ...
    while (running)
    {
        ...
        for (auto ver : scene.vertices) {
            glm::vec3 diff = glm::vec3(
                ver.position[0] - center_x,
                ver.position[1] - center_y,
                ver.position[2] - center_z
            );

            len_x = std::max(len_x, abs(glm::dot(diff, light_x)));
            len_y = std::max(len_y, abs(glm::dot(diff, light_y)));
            len_z = std::max(len_z, abs(glm::dot(diff, light_z)));
        }
        ...
        for (int i = 0; i < 3; i++) {
            transform[i][0] = light_x[i];
            transform[i][1] = light_y[i];
            transform[i][2] = light_z[i];
        }
        transform[0][3] = center_x;
        transform[1][3] = center_y;
        transform[2][3] = center_z;
        transform[3][3] = 1;
